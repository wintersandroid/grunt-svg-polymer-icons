 ...
 * https://github.com/wintersandroid/grunt-svg-polymer-icons
 *
 * Copyright (c) 2014 Mathew Winters
 * Modified from grunt-svgstore by Fabrice Weinberg
...
module.exports = function(grunt) {
  ...
  */
  ...
  grunt.registerMultiTask('svgpolymericon', 'Merge SVGs from a folder to a polymer element.', function() {
    ...
    var options = this.options({
      ...
      polymer: {
        iconsetImport: "/bower_components/core-iconset-svg/core-iconset-svg.html",
        iconsetElement: "core-iconset-svg.html"
      },
      symbol: {},
      formatting: false,
      includedemo: false,
    ...
    this.files.forEach(function(file) {
      ...
      file.src.filter(function(filepath) {
        ...
        // Generate symbol
        var $res = cheerio.load('<svg><g>' + $svg.html() + '</g></svg>', {
          xmlMode: true
        });
        var $symbol = $res('g').first();
        var $ssymbol = $res('svg').first();
        ...
        if (viewBox && !!options.fixedSizeVersion) {
          ...
          var $resFixed = cheerio.load('<symbol><use></use></symbol>', {
      ...
      var destName = path.basename(file.dest, '.html');
      $resultSvg.attr('id', destName);
